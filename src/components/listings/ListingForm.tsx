          ...
          {formData.type && <div className="bg-card rounded-lg border border-border p-5">
              ...
              {useUnitsMode ? <UnitsManager propertyType={formData.type as PropertyType} units={units} onUnitsChange={setUnits} /> : <div className="mb-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {getAvailableCategories().map(cat => <div key={cat.value} className={`p-3 border rounded-lg cursor-pointer transition-all ${formData.category === cat.value ? "border-primary bg-muted/40" : "border-border hover:border-primary/50 hover:bg-muted/30"}`} onClick={() => setFormData(prev => ({
                ...prev,
                category: cat.value
              }))}>
                        <div className="flex items-center gap-2">
